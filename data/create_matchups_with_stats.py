...
def main():
    ...
    matchups["WINNING_TEAM"] = matchups.apply(lambda row: 1 if row["WINNING_TEAM"] == row["TEAM1"] else 0, axis=1)
    ...
    matchups = matchups.merge(matchups_with_differentials[["TEAM1", "TEAM2", "DIFF"]], how="left", left_on=["TEAM1", "TEAM2"], right_on=["TEAM1", "TEAM2"])
    
    # Grab all team_stats{year}.csv files and concatenate them into one dataframe
    team_stats = pd.concat([pd.read_csv(f"team_stats_{year}.csv") for year in range(2006, 2025)])
    ...
    matchups = matchups[matchups["TEAM1"].isin(all_team_names_with_stats) & matchups["TEAM2"].isin(all_team_names_with_stats)]
    ...
    matchups = matchups.merge(team_stats.add_prefix("TEAM1_"), how="left", left_on="TEAM1", right_on="TEAM1_Team")
    ...
    matchups = matchups.merge(team_stats.add_prefix("TEAM2_"), how="left", left_on="TEAM2", right_on="TEAM2_Team")
    ...
    matchups = matchups.merge(seed_df.add_prefix("TEAM1_"), how="left", left_on="TEAM1", right_on="TEAM1_TEAM")
    matchups = matchups.merge(seed_df.add_prefix("TEAM2_"), how="left", left_on="TEAM2", right_on="TEAM2_TEAM")
    
    matchups = matchups.drop(columns="TEAM1_TEAM")
    matchups = matchups.drop(columns="TEAM2_TEAM")

    # Move the TEAM1_WIN column and the DIFF column to the end of the dataframe
    matchups = matchups[[col for col in matchups if col not in ["TEAM1_WIN", "DIFF"]] + ["TEAM1_WIN", "DIFF"]]
