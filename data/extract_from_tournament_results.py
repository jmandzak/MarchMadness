...
def remove_2025_teams(df: pd.DataFrame) -> pd.DataFrame:
    # Drop all rows that have 2025 as the value of the YEAR column
    df = df[df["YEAR"] != 2025]
    return df
...
def write_team_seeds_by_year(df: pd.DataFrame) -> None:
    ...
    pd.DataFrame(list(team_seeds_by_year.items()), columns=["TEAM", "SEED"]).to_csv(
        "team_seeds_by_year.csv", index=False
    )
...
def write_all_matchup_results(df: pd.DataFrame) -> None:
    ...
    new_df["WINNING_TEAM"] = np.where(
        df["SCORE"].iloc[::2].reset_index(drop=True)
        > df["SCORE"].iloc[1::2].reset_index(drop=True),
        new_df["TEAM1"],
        new_df["TEAM2"],
    )
...
def write_all_matchup_differentials(df: pd.DataFrame) -> None:
    ...
    new_df["TEAM1"] = df["TEAM"].iloc[::2].reset_index(drop=True)
    new_df["TEAM2"] = df["TEAM"].iloc[1::2].reset_index(drop=True)
    new_df["DIFF"] = df["SCORE"].iloc[::2].reset_index(drop=True) - df["SCORE"].iloc[
        1::2
    ].reset_index(drop=True)
    new_df.to_csv("all_matchup_differentials.csv", index=False)


def write_2025_matchups(df: pd.DataFrame) -> None:
    # Get the rows that have 2025 as the value of the YEAR column and have 64 for the CURRENT ROUND column
    ...
    new_df["TEAM1"] = df["TEAM"].iloc[::2].reset_index(drop=True)
    new_df["TEAM2"] = df["TEAM"].iloc[1::2].reset_index(drop=True)
    new_df.to_csv("2025_matchups.csv", index=False)
...
def main():
    ...
    write_2025_matchups(df)

    write_team_seeds_by_year(df)

    df = remove_2025_teams(df)
